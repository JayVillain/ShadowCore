# crawler.py (versi diperbaiki untuk debugging)
...
def get_fofa_data(query_string, page=1, size=100):
    ...
    try:
        ...
        print(f"Status Code: {response.status_code}") # <-- Tambahan
        
        if response.status_code == 200:
            data = response.json()
            if data['error']:
                print(f"ERROR dari API FOFA: {data['errmsg']}") # <-- Tambahan
                print(f"API Response Penuh: {data}") # <-- Tambahan
                return None
            return data
        else:
            print(f"ERROR: Status code {response.status_code}. Response: {response.text}") # <-- Tambahan
            return None
    except requests.RequestException as e:
        print(f"ERROR saat menghubungi API: {e}") # <-- Tambahan
        return None
    ...
    if data and 'results' in data:
        ...
        for item in data['results']:
            fofa_results.append({
                ...
                "header": item[3],
                "body": item[4],
                "app": "WordPress" if "wordpress" in str(item[2]).lower() or "wp-content" in str(item[4]).lower() else ""
            })
